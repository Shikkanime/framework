...
This file contains your core, non-negotiable root rules (règles mère). For detailed framework implementation guidance, refer to the linked documents in the `guidelines` directory as well as submodule-specific rules in each subfolder (`core/AGENTS.md`, `exposed/AGENTS.md`, `validator/AGENTS.md`, `ktor/AGENTS.md`).
...
- **Dependencies Flow:** `core` (standalone) <- `exposed` & `validator` <- `ktor`
...
Consult additional guides for [Database](guidelines/DATABASE.md) and [Performance](guidelines/PERFORMANCE.md) rules as needed.
